...
public class NotificationHandler {
    ...
    public static final String DELETE_BROADCAST = "org.hcilab.projects.nlog.delete";
    ...
    public void deleteLoggedValue(long id) {
        try {
            synchronized (LOCK) {
                String whereClause = DatabaseHelper.PostedEntry._ID + "= ?";
                String[] whereArgs = new String[]{String.valueOf(id)};

                DatabaseHelper dbHelper = new DatabaseHelper(context);
                SQLiteDatabase db = dbHelper.getWritableDatabase();
                db.delete(DatabaseHelper.PostedEntry.TABLE_NAME, whereClause, whereArgs);

            }
            Intent local = new Intent();
            local.setAction(DELETE_BROADCAST);
            local.putExtra(DatabaseHelper.PostedEntry._ID, id);
            LocalBroadcastManager.getInstance(context).sendBroadcast(local);
        } catch (Exception e) {
            if (Const.DEBUG) e.printStackTrace();
        }
